...
```{r odds_plot}
ggplot() +
  geom_function(fun = ~ .x / (1 - .x)) +
  xlim(0, 1) +
  theme_minimal() +
  labs(x = "p - probability of success", y = "odds")
```
...
```{r logit_plot}
ggplot() +
  geom_function(fun = ~ log(.x / (1 - .x))) +
  xlim(0, 1) +
  theme_minimal() +
  labs(x = "p - probability of success", y = "logit(p)")
```
...
```{r fit-logistic}
log_mod <- glm(survived ~ sex + age, data = titanic, family = "binomial")
tidy(log_mod)
```
...
- Holding sex constant, for every additional year of age, we expect the 
  log-odds of survival to decrease by approximately 0.002 on average.

- Holding age constant, we expect males to have a log-odds of survival that
  is 2.50 less than females on average.
...
For males and females the probability of survival decreases with age. Both
...
```{r practice_1}
logit_mod_2 <- glm(survived ~ sex * age, data = titanic, family = "binomial")

tibble(
  age = rep(0:80, times = 2),
  sex = rep(c("male", "female"), each = 81)
  ) %>% 
  augment(logit_mod_2, newdata = .) %>% 
...
```{r practice_2}
logit_mod_3 <- glm(survived ~ sex * age + factor(pclass), data = titanic, 
                   family = "binomial")
tidy(logit_mod_3)
```
